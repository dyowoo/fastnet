...
import (
	...
	"context"
	"fmt"
	"github.com/dyowoo/fastnet/xutils"
	"os"
	"path/filepath"
	"runtime"
	"sync"
...
func NewFastLog(prefix string, flag int) *FastLoggerCore {
	...
	log := &FastLoggerCore{prefix: prefix, flag: flag, isolationLevel: 0, calledDepth: 2}
	// 设置log对象 回收资源 析构方法(不设置也可以，go的Gc会自动回收，强迫症没办法)
	runtime.SetFinalizer(log, CleanFastLog)

	return log
}

// CleanFastLog Recycle log resources
...
*/
func (log *FastLoggerCore) formatHeader(t time.Time, file string, line int, level int) {
	var buf = &log.buf
...
func (log *FastLoggerCore) verifyLogIsolation(logLevel int) bool {
...
// 关闭日志绑定的文件
func (log *FastLoggerCore) closeFile() {
	if log.fw != nil {
		_ = log.fw.Close()
	}
}
